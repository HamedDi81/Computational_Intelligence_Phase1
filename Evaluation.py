...
def cluster_labels(x_pred , x_true):
    import numpy as np
    cluster_label = []                     #assign a value to each cluster
    for i in np.unique(x_pred):
        c = (x_pred == i)
        label, count = np.unique(x_true[c], return_counts=True)
        cluster_label.append(label[np.argmax(count)])
    y_pred = np.select([x_pred == i for i in np.unique(x_pred)],cluster_labels)
    return y_pred